...
public class CRC32 implements Checksum {
	...
	/*
	 * CRC LOOKUP TABLE ================ The following CRC lookup table was
	 * generated automagically by the Rocksoft^tm Model CRC Algorithm Table
	 * Generation Program V1.0 using the following model parameters:
	 * 
	 * Width : 4 bytes. Poly : 0x04C11DB7L Reverse : FALSE.
	 * 
	 * For more information on the Rocksoft^tm Model CRC Algorithm, see the
	 * document titled "A Painless Guide to CRC Error Detection Algorithms" by
	 * Ross Williams (ross@guest.adelaide.edu.au). This document is likely to be
	 * in the FTP archive "ftp.adelaide.edu.au/pub/rocksoft".
	 */
	static final int[] CRC_TABLE = {0x00000000, 0x04C11DB7, 0x09823B6E,
	...
	public void update(int b) {
		...
		// Process bytes starting from Least Significant byte

		update((byte) ((b) & byteMask));
		update((byte) ((b >> 8) & byteMask));
		update((byte) ((b >> 16) & byteMask));
		update((byte) ((b >> 24) & byteMask));
	}

	public void update(byte[] b, int off, int len) {
		int maxLength = off + len;
	...
	public static void main(String[] args) {
		...
		crc1.update("chinb".getBytes());
		
		crc2.update("chinb".getBytes());
		
		System.out.println(Long.toHexString(crc1.getValue()));
		System.out.println(Long.toHexString(crc2.getValue()));
	}
}
