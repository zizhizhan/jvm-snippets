...
@Slf4j
public class BlobEncryption implements Encryption, InitializingBean {
	...
	public String encrypt(List<Byte> inputBytes) {
		Assert.notEmpty(inputBytes, "null or empty input array");
	...
	public List<Byte> decrypt(String input) {
		...
		Assert.notNull(input, "inputString is null");
		...
		Assert.isTrue(inputBytes.length > 12,
				"invalid input, < 13, no encoded chars");

		Assert.isTrue(inputBytes[0] == 'E' && inputBytes[1] == 'X' 
			&& inputBytes[2] == '0' && inputBytes[3] == '1',
				"invalid input, doesn't being with Expedia signature");
	...
	public void afterPropertiesSet() throws Exception {
		Assert.notNull(charJumble, "charJumble not injected");
	}
